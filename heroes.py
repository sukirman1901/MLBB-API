...
def transform_skill_data(skill: Dict) -> Dict:
    ...
    scaling = skill.get('scaling') or skill.get('skill_scaling')
    if isinstance(scaling, dict):
        if 'cooldown' in scaling and scaling['cooldown'] is not None:
            cd = scaling['cooldown']
            cooldown = " / ".join(str(x) for x in cd) if isinstance(cd, list) else str(cd)
        if 'mana_cost' in scaling and scaling['mana_cost'] is not None:
            mc = scaling['mana_cost']
            manacost = " / ".join(str(x) for x in mc) if isinstance(mc, list) else str(mc)

    s_name = skill.get('name') or skill.get('skill_name') or ''
    s_icon = skill.get('image_path') or skill.get('skill_image_path') or ''
    s_desc = skill.get('description') or skill.get('skill_description') or ''
...
def enrich_heroes(hero_meta: Dict, hero_details: Dict[str, Dict]) -> Dict:
    ...
    for hero in hero_meta['data']:
        ...
        # Add baseStats
        if 'baseStats' in detail_data and detail_data['baseStats']:
            hero['base_stats'] = detail_data['baseStats']
...
def main():
    ...
    hero_meta_path = 'v1/hero-meta-final.json'
